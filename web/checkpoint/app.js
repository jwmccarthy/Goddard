...
function makeCar(color) {
  const geometry = new THREE.BoxGeometry(120, 87, 39);
...
const ball = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ color: 0xe9edf2 })
);
scene.add(ball);
...
source.onmessage = ({ data }) => {
  ...
  goals.textContent = `${frame.blue_score} - ${frame.orange_score}`;
  status.textContent = `${frame.checkpoint} | latent steps ${frame.latent_steps.join(', ')}`;
};
