...
def main() -> None:
    ...
    try:
        ...
        runner, rollout, ppo = build_ppo(environment, policy, value_function, arguments)
        run_name = arguments.run_name or datetime.now().strftime(
            "goddard-%Y%m%d-%H%M%S"
        )
        run_dir = arguments.log_dir / run_name
        trainer = Trainer(
            runner,
            rollout,
            ppo,
