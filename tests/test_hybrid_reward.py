...
class HybridRewardTests(unittest.TestCase):
    ...
    def reward_context(
        ...
        *,
        ...
        events = CarlEvents(
            ...
            truncated=torch.tensor([done and not bool(score_delta)]),
        )
        ...
        # These fields are supplied by current CARL environments but are not
        # constructor fields in older CARL releases.
        object.__setattr__(
            context, "score_difference", torch.tensor([score_difference])
        )
        object.__setattr__(
            context, "episode_ticks", torch.tensor([episode_ticks])
        )
        object.__setattr__(context, "overtime", torch.tensor([overtime]))
        return context
    ...
    def test_nexto_mechanics_reward_components(self):
        ...
        result = SeerReward(
            ...
            weights=SeerRewardWeights(aerial_touch=1.0),
        )(
    ...
    def test_ground_pop_can_become_a_qualified_air_dribble(self):
        reward = SeerReward(1, 1, normalize=False)
        blue = 9
        previous = self.raw_state()
        current = previous.clone()
        previous[:, 2] = 180.0
        previous[:, blue + 16] = 1.0
        current[:, 2] = 220.0
        current[:, 5] = 500.0
        current[:, blue + 16] = 1.0
        current[:, blue + 21] = 1.0

        reward(self.reward_context(previous, current, episode_ticks=0, done=False))

        self.assertTrue(reward._air_setup_active[0, 0])
        self.assertFalse(reward._air_active[0, 0])

        previous = current
        current = previous.clone()
        current[:, 2] = 300.0
        current[:, blue + 2] = 180.0
        current[:, blue + 16] = 0.0
        current[:, blue + 21] = 0.0
        reward(self.reward_context(previous, current, episode_ticks=4, done=False))

        previous = current
        current = previous.clone()
        current[:, 2] = 450.0
        current[:, blue + 2] = 320.0
        current[:, blue + 21] = 1.0
        reward(self.reward_context(previous, current, episode_ticks=8, done=False))

        self.assertTrue(reward._air_active[0, 0])
        self.assertEqual(reward._air_contacts[0, 0], 1.0)

        previous = current
        current = previous.clone()
        current[:, 2] = 520.0
        current[:, blue + 2] = 390.0
        current[:, blue + 21] = 0.0
        reward(self.reward_context(previous, current, episode_ticks=16, done=False))

        previous = current
        current = previous.clone()
        current[:, 2] = 600.0
        current[:, blue + 2] = 470.0
        current[:, blue + 21] = 1.0
        reward(self.reward_context(previous, current, episode_ticks=24, done=False))

        self.assertTrue(reward._air_qualified[0, 0])
        self.assertEqual(reward._air_contacts[0, 0], 2.0)

    def test_wall_release_starts_wall_air_dribble_route(self):
        reward = SeerReward(1, 1, normalize=False)
        blue = 9
        previous = self.raw_state()
        current = previous.clone()
        ...
        current[:, 5] = 450.0
        current[:, blue] = 4000.0
        current[:, blue + 2] = 400.0
        current[:, blue + 16] = 0.0
        current[:, blue + 21] = 1.0

        reward(self.reward_context(previous, current, episode_ticks=12, done=False))

        self.assertTrue(reward._air_active[0, 0])
        self.assertTrue(reward._air_wall_route[0, 0])
        self.assertEqual(reward._air_contacts[0, 0], 1.0)

    def test_low_aerial_touch_does_not_start_air_dribble(self):
        reward = SeerReward(1, 1, normalize=False)
        blue = 9
        previous = self.raw_state()
        current = previous.clone()
        previous[:, 2] = 280.0
        current[:, 2] = 300.0
        current[:, blue + 2] = 220.0
        current[:, blue + 16] = 0.0
        current[:, blue + 21] = 1.0

        reward(self.reward_context(previous, current, episode_ticks=12, done=False))

        self.assertFalse(reward._air_setup_active[0, 0])
        self.assertFalse(reward._air_active[0, 0])
